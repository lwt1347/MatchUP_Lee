...
<html>
 ...
 require("../mainhtml.php");
  ...
  <!-- ajax 를 통한 서버와의 통신  -->
        ...
        <!--이미지 서버에 전송 하기 로직-->
        <form  action="playerNickNameInsert.php" method="POST" enctype="multipart/form-data">
...
<div class="collapse-navbar-collapse">
           ...
           <tr>
               <td>
                 ...
                 E-mail : <input type="text" name = "emailName" value = "<?php 
          ...
          <tr>
            <td colspan="2">
              <label for="inputEmail" class="sr-only" >닉네임을 입력하세요.</label>
              <input class="form-control" id = "nickName" name = "nickName" placeholder="닉네임을 입력하세요." required>

              <label id="checkNickName">NickName Check.</label>
...
<!-- // 다음 주소검색 창 -->
 ...
 <td colspan="2">
           ...
           <tr>
             <td colspan="2">
              <label for="inputEmail" class="sr-only" >자신의 번호를 입력하세요.</label>
              <input  type="number" class="form-control" id = "number" name = "number" placeholder="자신의 번호를 입력하세요."  min="1" max="99">
            </td>

          </tr>
          <tr>
             <td colspan="2">
              <label for="inputEmail" class="sr-only" >자신의 포지션을 입력하세요.</label>
              <input  class="form-control" id = "position" name = "position" placeholder="자신의 포지션을 입력하세요." >
            </td>
          ...
          <tr>
            <td>
              ...
              <input type="submit" value="선수로 등록하기" id="btn_submit" disabled = false style="color : gray" onclick="pictureName()"/>
        ...
        <script>
         var check;
          document.querySelectorAll('input')[4].addEventListener('focusout', function(event){
            ...
            xhr.open('POST', './playerNickNameCheck.php');
...
//팀이름 2 ~ 14 사이만 입력 가능  
if(document.getElementById("nickName").value.length > 1 && document.getElementById("nickName").value.length <=15){


                if(checkTeamInnerHTML > 0){
                  checkTeamInnerHTML = "<span style='color:green'>사용 가능한 닉네임입니다.</span>";
                  //btn.disabled = false;
                  ...
                  checkTeamInnerHTML = "<span style='color:red'>닉네임이 중복됩니다.</span>";
                  btn.disabled = true;
                  btn.style.color = "gray";
                  teamCb = false;
                }

                document.querySelector('#checkNickName').innerHTML = checkTeamInnerHTML;
  ...
  checkTeamInnerHTML = "<span style='color:red'>닉네임 명을 2~15자리 사이를 입력하세요.</span>";
  document.querySelector('#checkNickName').innerHTML = checkTeamInnerHTML;
  btn.disabled = true;
  btn.style.color = "gray";
  teamCb = false;
            ...
            var data = ''; //서버로 보낼 데이터
            data += '&format='+ document.getElementById("nickName").value;
            xhr.send(data); 
